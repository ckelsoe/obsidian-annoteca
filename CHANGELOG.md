...
- Another plugin can now add comments to your notes through Annoteca, and only add them. There is no way for it to resolve, delete or edit a comment, because deciding a comment is dealt with is a judgement about your writing. If it adds more than ten at once you are asked first, with the count and the note named. Adding the same finding twice does nothing, so a plugin re-checking a note it has already commented on will not fill it with duplicates.
