...
- Writing about the comment format inside a comment no longer changes the comment. Details are stored at the end of a comment, one per line, so a comment whose text ended in a line of that shape was read as carrying that detail, and the line was removed from what you wrote. A body ending in `[resolved bob 2026-01-01]: done` silently marked the comment resolved; `[author=someone]` silently changed who it was from; `[reply ...]` added a reply nobody wrote. Those lines are now stored so the plugin can tell them apart from the real thing, and they come back exactly as typed. Comments already damaged this way cannot be recovered by the fix; the line left the file when it was last written.
...
- The exported AI skill now teaches the two remaining rules for writing a comment by hand: how to write `<!--` inside one, and how to write a line that looks like one of the comment's own detail lines. An assistant hits both without trying, the first by quoting the format while explaining it. The skill's schema version has gone up, so the plugin will tell you to re-export it with "Export AI skill".
