...
- The read-only API can now list your comment categories, so a companion plugin creating comments can offer the same choices you see instead of guessing at a hardcoded list that goes stale when you add or rename one. It returns the id and the display name only; icons and colours stay internal.
- The read-only API's anchor ranges now say whether a comment is `addressed`, meaning a proposed edit is sitting in the note awaiting accept, revise or reject. `resolved` could not answer that on its own, because an addressed comment is still open. A companion plugin that dims its own marks under your open comments needs the difference: a passage with a proposal waiting on you is back in play.
- A "Findings" tab in the hub, when Plumbline is installed. It lists that plugin's prose findings for the note you are in: ranked worst-first, errors and warnings as their own rows with suggestions below them, each naming the rule that fired and the line it is on. Clicking a row jumps to the words in the editor; "Add comment" turns the finding into a real comment you can answer, and pressing it twice does nothing rather than leaving two. The lane updates as you type. Nothing is written to your note until you press that button, and the tab is absent entirely if Plumbline is not installed, so a solo Annoteca vault looks exactly as it did.
- A "Prose check" category, for comments a prose linter promotes into your notes. It is always available and has no toggle in front of it, because the marker it writes lives in the note itself: someone you share a file with needs the category to resolve even if they never installed the linter that created it. Your own naming wins if you already have a category with that id.
- A setting under AI integration controls how many comments another plugin may add at once before you are asked. Ten by default, anywhere from 1 to 500. At 1 you are asked whenever a plugin adds more than one comment; a single comment is never confirmed.
- Another plugin can now add comments to your notes through Annoteca, and only add them. There is no way for it to resolve, delete or edit a comment, because deciding a comment is dealt with is a judgement about your writing. If it adds more than ten at once you are asked first, with the count and the note named. That threshold is a setting. Adding the same finding twice does nothing, so a plugin re-checking a note it has already commented on will not fill it with duplicates.
- Comments can now record that another plugin created them, not a person, with a `[source=...]` line naming the plugin and its own reference for the finding. The exported AI skill (now version 8) teaches the line and the rule that comes with it: an assistant must never resolve a machine-created comment on your behalf. A finding stops being reported the moment the prose changes, and that is not the same as the prose being right, so resolution stays your call. If a finding is no longer detected, the assistant replies saying so rather than removing the comment.
...
- The Outline tab now marks the heading your cursor sits in for a note shown in a tab you have not opened since Obsidian started. Those tabs are restored without an editor and only filled in when you first look at them, so the outline had nothing to read the cursor from and left the heading unmarked. It now loads that tab's note once, waits for it to fill, and marks the heading, and it does this only while you are looking at that note so a tab you have since moved away from is not repainted.
- Clicking one comment and then another in quick succession now lands on the second one. Each click waits for the note's tab to be ready before it moves the cursor, and a later click that started during that wait could finish first, leaving the earlier click's comment as where you ended up. Clicks are now handled one after another, so the last one you click is where you land.
- The Hub keeps its selection on a comment made by "Convert comments" when you edit the text above it. Those comments carry no identifier, so the panel could only recover the selection after such an edit by identifier, and fell back to the first comment in the list while the editor kept highlighting the one you chose. It now recovers these by their category and text, the same key it already uses to remember which cards you expanded.
...
### Added
- Annoteca tells you when a note has a broken comment marker, once per note, as you open or save it. The check existed but only ran when you went looking for it under "Validate marker format", and the problems it finds are the ones that hide your text without anything looking wrong, so nobody went looking until it had already cost them a paragraph.
...
### Changed
- A comment written by an earlier version whose text quotes a marker opener now reads wrongly. To tell a marker that lost its `-->` from one that is merely quoting the format, the plugin now escapes `<!--` when it writes a comment, and reads an unescaped one as a marker that never closed. Comments already in your notes were written before that escape existed, so a body containing `<!-- annoteca/todo:` is the same text either way. Such a comment now comes back under the category it was quoting, with its text cut down to whatever followed the quote, while still carrying the original comment's identifier. Nothing is deleted and nothing is rewritten on its own: the note reports a broken marker as soon as it is read, and removing a comment inside the stretch that marker hides is refused while it stands. The repair is to escape the inner opener by hand as `\<!--`. The report calls this a missing `-->`, because from the outside the two are the same text, but adding one is not the fix here.
...
### Fixed
- Saving a comment no longer writes it into whichever note the tab has moved on to. The Add and Edit forms open beside the editor and leave it usable on purpose, and Obsidian hands a tab's editor over to whatever file that tab shows next, so clicking a link, clicking a note in the sidebar, or using "Make a copy" while the form was open sent Save into the new note. The comment landed in a note you were not commenting on, nothing said so, the note you meant to write to was left untouched, and the panel refreshed as though it had worked. Saving now checks that the tab is still showing the note the form was opened on, and says so and stays open rather than writing somewhere else. This covers writing a new comment as well as editing one; only editing had any check at all before, and it ran when the form opened rather than when you pressed Save.
- Editing a comment that has no identifier no longer lands on a different comment. Comments produced by "Convert comments" carry no identifier, so they are told apart by where they sit, which category they are, and how long they are. A run of them differs only in wording, and two of the same length matched on all three, so deleting a line above the one being edited while the form was open slid the next comment into its place: Save overwrote that one instead, destroying its text, the edit never reached the comment it was meant for, and no warning appeared. What the comment says is now part of what identifies it, which is what every other action already required. Two comments that are word-for-word identical still cannot be told apart, but nothing is lost in that case, because the two carry the same text either way.
- A collaborator colour set for a name with a space in it now applies. Names are folded into a form the comment format can hold, with spaces becoming hyphens, whenever they are written into a note, but a name stored in the settings file was kept exactly as found. The colour was filed under `Bob Smith` while every comment in the note said `Bob-Smith`, so the colour you chose silently never appeared and the reply box offered both spellings. Names are now folded the same way wherever they arrive, including from a settings file edited by hand or arriving over sync, and two spellings that fold to the same name collapse into one entry keeping the colour.
- A comment missing its closing `-->` no longer swallows the paragraph below it. The plugin paired that comment's opening with the NEXT comment's closing, so everything in between, your prose included, became one comment's contents: the paragraph vanished from reading view, the second comment vanished from the panel, and the merged result carried the second comment's identifier under the first one's category. Deleting it removed the paragraph from the note. A closing now belongs to the last comment opened before it, so the second comment reads normally and your prose stays outside both.
- Writing about the comment format inside a comment no longer changes the comment. The lines that carry a comment's identifier, date, author, and anchor sit at the end of the marker, one per line, so a comment whose text ended in a line of that shape was read as carrying that detail, and the line was removed from what you wrote. A body ending in `[resolved bob 2026-01-01]: done` silently marked the comment resolved; `[author=someone]` silently changed who it was from; `[reply ...]` added a reply nobody wrote; and a body ending `[id=...]` came back carrying that identifier instead of its own, which orphaned its star, its draft, and every later action. A comment's own details now win, and a line you merely wrote is stored so the plugin can tell the two apart, and comes back exactly as typed. Comments already damaged this way cannot be recovered by the fix; the line left the file when it was last written.
- A detail the plugin does not recognise inside a comment is no longer deleted. Comments written by a future version of Annoteca, or by hand, can carry fields this version has no name for, and the format has always promised those are ignored rather than rejected. They were being ignored and then dropped: the next time anything touched the comment, the line was gone. They are now kept exactly as written and put back.
- Deleting a comment that sits inside the stretch hidden by a comment missing its closing `-->` is refused. That closing is the only thing ending the hidden stretch, so removing it hides more of the note and nothing about the note looks different afterwards. The plugin says what is wrong and what to type to fix it. Comments further down the note, past the point where the hidden stretch ends, are unaffected and delete normally; and resolving, replying and editing still work everywhere, because they put a comment back where a comment was.
- Converting comments in bulk no longer rewrites text that is hidden inside an unclosed comment. Anything between a comment missing its `-->` and the next one is invisible in reading view, so a conversion there is a change to your note that nobody can see.
- The exported AI skill now teaches every rule an assistant needs in order to write a comment by hand. It covers escaping `-->`, which ends the marker early and loses everything after it; escaping `<!--`, which an assistant hits without trying the moment it quotes the format while explaining it; writing a line that looks like one of the comment's own detail lines; and what an author name has to look like. The name rule was the one that read as cosmetic: the skill gave the shape but never said that a name the format cannot hold makes the whole line unreadable, so an assistant signing as `Bob Smith` lost the comment's author, or, in a reply, the identifier and the entire thread. It now says to write the name as one word joined with hyphens, and why. The skill's schema version has gone up, so the plugin will tell you to re-export it with "Export AI skill".
- Converting comments in bulk no longer rewrites the examples inside an indented code block, inside a code fence nested in a list item, or inside a code block in a quoted passage, and no longer counts them as conversions. Any code block whose content sat four or more columns in was unprotected, which covers a fence under a `10.` step, under a `- [ ]` task, in a vault that indents lists by four spaces, and every plain four-space or tab-indented block. A note documenting comment syntax had its samples destroyed and the notice reported them as successes.
- Converting comments in bulk no longer skips everything after an unterminated code fence in a quoted passage or a list item. A fence that was never closed was treated as running to the end of the note, so every real comment below it was passed over and the count reported them as if they had not been there. A fence now ends with the quote or the list item that holds it, which is where a reader sees it end.
- Converting comments in bulk no longer overwrites a change you make while it runs. It used to read a note, convert it, and write the whole file back without checking whether anything had landed in between, so resolving a comment during the sweep could put the marker back exactly as it was while the notice still said "Resolved." The conversion now happens inside the write, so what the count reports is what reached the file, and a note open in a pane is written through the editor instead of underneath it.
- "Clear orphaned stars" no longer deletes stars whose notes are perfectly live. The vault scan behind it ran once per session and nothing told the plugin about a note that arrived afterward, so a note synced in from another device and not opened looked deleted, and its starred comments were removed with the message "Cleared 2 orphaned stars." Drift-check baselines were being discarded the same way. Notes that appear while Obsidian is running are now picked up as they arrive.
- The exported AI skill no longer breaks when the reviewer's author tag holds a backtick. The tag is shown as code in the skill text, and a backtick in it ended that code span early, so everything after it in the line rendered wrong. The tag's wrapper now grows past any run of backticks inside it, the same rule the stored-original fence already follows.
...
- Importing comments no longer rewrites text inside an existing comment, a code block, or an inline code span. A `%%comment%%` written inside the body of an Annoteca comment was converted where it stood, which put a comment inside a comment: the outer one ended early and the rest of it appeared in the note as visible text. "Convert every comment to the canonical format" hit this every time, because it converts `%%...%%` first. The same pass also converted the examples in a note that documented comment syntax, destroying the sample and counting it as a success. Import now leaves all three kinds of region alone, and running it twice over the same note changes nothing the second time. A code span that runs across a line break counts as one too, which is how Markdown reads it and is a shape that turns up whenever a long snippet is wrapped.
...
- "Validate marker format" now reports a comment whose closing `-->` is missing. Without it, that comment pairs with the NEXT comment's closing tag: everything between them, prose included, is swallowed into one comment, the prose disappears from reading view, and the second comment disappears from the Hub. The document still reads the same way, so this is a report rather than a change in behaviour, but it is now something you can see and fix instead of noticing the missing paragraph much later.
...
- The Hub panel redraws when a display setting changes. Changing how comments are shown left every card as it was until something unrelated happened to redraw the panel, so the Hub went on showing the setting you had just moved away from.
