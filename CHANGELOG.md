...
- Deleting a comment is refused in a note that has a comment missing its closing `-->`. That closing is the only thing ending the hidden stretch the unclosed comment opened, so removing it hides more of the note, and nothing about the note looks different afterwards. The plugin now says so and points at "Validate marker format" instead. Resolving, replying and editing still work, because they put a comment back where a comment was.
