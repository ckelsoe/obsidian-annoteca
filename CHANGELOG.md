...
### Fixed
- Converting comments in bulk no longer rewrites the examples inside an indented code block or inside a code fence nested in a list item, and no longer counts them as conversions. Any code block whose content sat four or more columns in was unprotected, which covers a fence under a `10.` step, under a `- [ ] ` task, in a vault that indents lists by four spaces, and every plain four-space or tab-indented block. A note documenting comment syntax had its samples destroyed and the notice reported them as successes.
...
- "Clear orphaned stars" no longer deletes stars whose notes are perfectly live. The vault scan behind it ran once per session and nothing told the plugin about a note that arrived afterwards, so a note synced in from another device and not opened looked deleted, and its starred comments were removed with the message "Cleared 2 orphaned stars." Drift-check baselines were being discarded the same way. Notes that appear while Obsidian is running are now picked up as they arrive.
