...
export class StarredTabRenderer {
	...
	render(container: HTMLElement): void {
		...
		if (ids.size === 0) {
			this.renderEmpty(
				container,
				'No starred comments yet. Hover a comment marker and click the star to add one.',
			);
			return;
		}
		...
		for (
			let i = this.plugin.settings.starredComments.length - 1;
			i >= 0;
			i--
		) {
		...
		for (const entry of ordered) {
			...
			card.createDiv({
				cls: 'annoteca-starred-body',
				text: truncate(c.body, 160),
			});
