...
export const MODAL_TEMPLATES: Record<string, ModalTemplate> = {
	...
	'source-needed': {
		fields: [
			...
			{
				id: 'claim',
				label: 'Claim needing source',
				placeholder: 'what needs a source?',
				type: 'textarea',
			},
		],
		compose(values, body) {
	...
	'index-entry': {
		fields: [
			...
			{
				id: 'subterm',
				label: 'Subterm (optional)',
				placeholder: 'e.g., aspect of the topic',
			},
		],
		compose(values, body) {
...
export function resolvePlaceholder(
	...
	if (
		field.id === 'term' &&
		ctx.selectedText &&
		ctx.selectedText.trim().length > 0
	) {
...
export function composeScriptureReference(
